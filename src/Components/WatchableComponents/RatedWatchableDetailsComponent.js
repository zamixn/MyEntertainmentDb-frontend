...
class RatedWatchableDetailsComponent extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    const url = Constants.getRatedWatchableAPI_URL(this.props.id);
  ...
  render() {
    ...
    if (this.state.data) {
      ...
      const watchable = this.state.data.watchable.watchable;
      const creator = this.state.data.watchable.creator;
        ...
        <div>
          ...
          <table className="center">
            <tbody>
              <tr className='no-border'>
                ...
                <td></td>
                <td>Type: {EnumFormatter.formatWatchableType(watchable.type)}</td>
              </tr>
            </tbody>
          </table>
          <br />
          <table className="center">
            <tbody>
              <tr className='no-border'>
                <td>Times seen:   </td>
                <td> <input className='numberInputField' pattern="[0-9]*" id={'times_'+watchable.id} type='number' value={rating.timesConsumed} onChange={(e) => this.timesChanged(e, this.state)}/> </td>
                <td></td>
                <td></td>
                <td>Last seen:   </td>
                <td> <input className='dateInputField' id={'last_'+watchable.id} type='date' value={format(new Date(rating.lastConsumed), 'yyyy-MM-dd')} onChange={(e) => this.lastChanged(e, this.state)}></input> </td>
              </tr>
              <tr className='no-border'>
                <td>Rating:   </td>
                <td>  <input className='numberInputField' id={'rating_'+watchable.id} type='number' value={rating.rating} onChange={(e) => this.ratingsChanged(e, this.state)}/> </td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
              ...
              <tr className='no-border'>
                ...
                <td></td>
                <td></td>
                <td> <input id={watchable.id} type='button' value='save' onClick={(e) => this.saveChanges(e, this.state)}/> </td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default RatedWatchableDetailsComponent;